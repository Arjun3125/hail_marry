...
- **Students** — get AI-powered study help grounded in their syllabus
- **Teachers** — upload materials, track performance, get class analytics
- **School Administrators** — manage users, monitor AI usage, ensure quality
- **Parents** (Phase 2) — view child's performance
...
- Student portal: 11 routes (`/student/overview`, `/ai`, `/attendance`, `/results`, `/assignments`, `/timetable`, `/lectures`, `/complaints`, `/profile`, `/tools`, `/upload`)
- Teacher portal: 8 routes (`/teacher/dashboard`, `/classes`, `/attendance`, `/marks`, `/assignments`, `/upload`, `/insights`, `/profile`)
...
- Parent portal: 4 routes (`/parent/dashboard`, `/attendance`, `/results`, `/reports`)
- Login page
...
| **Prompt Templates** | 8 modes: Q&A, Study Guide, Quiz, Concept Map, Weak Topic, Flowchart, Mindmap, Flashcards | ✅ Exceeds docs (docs list 5, code has 8) |
...
**Models implemented:** `Tenant`, `User`, `Class`, `Subject`, `Enrollment`, `Attendance`, `Exam`, `Mark`, `Assignment`, `AssignmentSubmission`, `Timetable`, `Complaint`, `Lecture`, `Document`, `AIQuery`, `AuditLog`, `SubjectPerformance`, `ParentLink`, `WebhookSubscription`, `WebhookDelivery`
...
Student submits query via /api/ai/query
  ...
  → Prompt template filled (8 mode-specific templates)
...
| **Parent Portal** | Read-only child data view (7.4KB routes) | Parent | Parent-child linking requires admin setup |
...
| Test coverage | 🔴 Missing | `backend/tests/` has 1 file; `frontend/tests/` has 1 file |
...
| **Parents** (Phase 2) | Visibility into child's performance and AI-assisted learning |
...
|---|---|---|
| 1 | **Reduce AI modes from 8 to 4 for pilot** (Q&A, Study Guide, Quiz, Weak Topic) | Reduces LLM prompt engineering surface; easier to QA; faster iteration |
