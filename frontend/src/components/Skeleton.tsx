...
export function SkeletonCardGrid({ items = 6, className = "" }: { items?: number; className?: string }) {
    const fallback = (
        <div className={`grid gap-4 ${className} sm:grid-cols-2 lg:grid-cols-3`}>
            {Array.from({ length: items }).map((_, i) => (
                <div key={i} className="prism-skeleton-card rounded-2xl border border-[var(--border)] bg-[rgba(148,163,184,0.05)] p-5 space-y-3">
                    <div className="flex items-center justify-between">
    ...
    return (
        <BoneyardSkeleton
            loading
            name="prism-skeleton-card-grid"
            className={`grid gap-4 ${className} sm:grid-cols-2 lg:grid-cols-3`.trim()}
            fallback={fallback}
            fixture={fallback}
        >
            {fallback}
        ...
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="rounded-2xl border border-[var(--border)] bg-[rgba(148,163,184,0.05)] p-4 space-y-2">
                        <Skeleton className="h-3 w-20" />
            ...
            {/* Main content */}
            <div className="rounded-2xl border border-[var(--border)] bg-[rgba(148,163,184,0.05)] p-6 space-y-4">
...
export function SkeletonQuiz({ className = "" }: { className?: string }) {
    const fallback = (
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="space-y-3">
                {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="rounded-2xl border border-[var(--border)] bg-[rgba(148,163,184,0.05)] p-4">
