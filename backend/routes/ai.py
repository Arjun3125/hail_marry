...
class AIQueryRequest(BaseModel):
    ...
    language: str = "english"  # english, hindi, tamil, telugu, kannada, bengali, marathi, gujarati
    response_length: str = "default"  # brief, default, detailed
    expertise_level: str = "standard"  # simple, standard, advanced
...
async def ai_query(
    ...
    # ── Inject Language & Style ────────────────────────
    style_parts = []
    lang = request.language.strip().lower()
    if lang and lang != "english":
        style_parts.append(f"Respond entirely in {lang}.")
    length_instr = LENGTH_INSTRUCTIONS.get(request.response_length, "")
    if length_instr:
        style_parts.append(length_instr)
    expertise_instr = EXPERTISE_INSTRUCTIONS.get(request.expertise_level, "")
    if expertise_instr:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                ...
                json={
                    ...
                    "options": {
                        "temperature": 0.1,
                        "num_predict": LENGTH_TOKENS.get(request.response_length, 800),
                    },
                },
            )
