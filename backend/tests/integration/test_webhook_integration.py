...
@pytest.mark.asyncio
async def test_concurrent_webhook_deduplication(monkeypatch):
    """
    ...
    whatsapp_gateway.WHATSAPP_APP_SECRET = "test_app_secret"
    ...
    try:
        ...
        async def single_request(client_session):
            response = await client_session.post("/api/whatsapp/webhook", content=body_bytes, headers=headers)
            return response
