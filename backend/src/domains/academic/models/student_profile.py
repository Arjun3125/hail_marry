...
class StudentProfile(Base):
    ...
    current_class_id = Column(UUID(as_uuid=True), ForeignKey("classes.id"), nullable=True, index=True)
    current_batch_id = Column(UUID(as_uuid=True), ForeignKey("batches.id"), nullable=True, index=True)
    primary_parent_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
    guardian_count = Column(Integer, default=0)

    # ── Attendance aggregates ────────────────────────────────────────
    total_days = Column(Integer, default=0)
    present_days = Column(Integer, default=0)
    attendance_pct = Column(Float, default=0.0)      # Computed: present_days / total_days * 100
    absent_streak = Column(Integer, default=0)        # Consecutive absent days (risk flag)
    ...
    subject_mastery_map = Column(JSONB, nullable=True)

    # ── Spaced Repetition / Learning streaks ─────────────────────────
    current_streak_days = Column(Integer, default=0)  # Daily learning streak
    ...
    dropout_risk = Column(String(20), default="low")   # low, medium, high
    academic_risk = Column(String(20), default="low")  # low, medium, high
    fee_risk = Column(String(20), default="low")       # low, medium, high (from fee module)
