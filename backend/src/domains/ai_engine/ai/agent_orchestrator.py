...
from src.domains.ai_engine.ai.erp_tools import check_library_catalog, get_school_library_stats, get_school_financial_report
...
def plan_node(state: AgentState) -> dict:
    ...
    llm_json = ChatOllama(model="llama3.2", format="json", temperature=0.1)
...
- 'mode': one of 'qa', 'quiz', 'study_guide', 'concept_map' (if continue)
- 'prompt_template': the specific query for this step (if continue)
"""
    try:
        res = llm_json.invoke([SystemMessage(content=sys_prompt)])
        data = json.loads(res.content)
        if data.get("action") == "finish":
            return {"status": "completed", "pending_mode": None, "pending_prompt": None}
            
        return {
            "pending_mode": data.get("mode", "qa"),
            "pending_prompt": data.get("prompt_template", topic)
        }
    except Exception:
